...
from tqdm.notebook import tqdm
from networks import ConvNet, ResNet50
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    ...
    if model == 'MLP':
        ...
        net = ResNet50(in_channels=channel, n_classes=num_classes)
...
def epoch(mode, dataloader, net, optimizer, criterion, dsa_params, device, aug, texture=False, If_Float = True):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if aug:
            img = DiffAugment(img, strategy = 'color_crop_cutout_flip_scale_rotate', param = dsa_params)
...
class MttDataset(Dataset):
    ...
    def __init__(self, imgs, labels):
        ...
        self.pics = [i for i in range(0, self.__len__())]

        for i in range(0, self.__len__()):
            self.pics[i] = self.labels[i]
            self.idxs[i] = self.imgs[i]
...
def epoch_dist(mode, dataloader, net, big_net, optimizer, criterion, dist_criterion, dsa_params,
               device, weight = 0.5, temp = 1.0, aug = False, If_Float = True):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        img = img.to(device)
        n_b = lab.shape[0]
        soft_label = F.softmax(big_net(img)/temp)
        output = net(img)
        soft_output = F.softmax(output/temp)
        loss = (1 - weight) * criterion(output, lab) + weight * dist_criterion(soft_label, soft_output)
...
def study_teacher(model, trainloader, testloader, dsa_params, device, train_criterion = None):
    ...
    for ep in tqdm(range(epochs)):
        # training loop
        loss_avg, acc_avg = epoch('train', trainloader, model, optimizer, train_criterion, dsa_params, device, aug = True, If_Float = False)
        train_loss.append(loss_avg)
        train_acc.append(acc_avg)
        # validation loop
        val_loss_avg, val_acc_avg = epoch('test', testloader, model, optimizer, test_criterion, dsa_params, device, aug = False, If_Float = False)
...
def study_student_with_teacher(syn_imgs, big_model, model, trainloader, testloader, dsa_params, device, weight=0.5, temp=1.0):
    match syn_imgs:
        case 'mtt':
            soft_cri = nn.CrossEntropyLoss()
            If_Float = False
        case 'datm':
    ...
    for ep in tqdm(range(epochs)):
        # training loop
        loss_avg, acc_avg = epoch_dist('train', trainloader, model, big_model, optimizer, soft_cri, loss_distil, dsa_params, device, weight = 0.5, temp = 0.5, aug = True, If_Float=If_Float)
        train_loss.append(loss_avg)
        train_acc.append(acc_avg)
        # validation loop
        val_loss_avg, val_acc_avg = epoch('test', testloader, model, optimizer, test_criterion, dsa_params, device, aug = False, If_Float = False)
        val_loss.append(val_loss_avg)
        val_acc.append(val_acc_avg)
        print(f'loss: {loss_avg:.4f}, acc: {acc_avg:.4f}, val loss: {val_loss_avg:.4f}, val acc: {val_acc_avg:.4f}')

    return train_loss, train_acc, val_loss, val_acc

def get_synset_loader(dataset, syn_imgs, ipc):
    path = './' + syn_imgs + '/' + dataset.lower() + '/' + str(ipc) + 'ipc'
    subprocess.call(f'./{syn_imgs}.sh')

    # загрузить тензоры нужные
    images_train = torch.load(path + '/images_best.pt')
    labels_train = torch.load(path + '/labels_best.pt')
    match syn_imgs:
        case 'mtt':
            dataset = MttDataset(images_train, labels_train)
        case 'datm':
